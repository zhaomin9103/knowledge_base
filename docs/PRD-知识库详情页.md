...
> **v4.0 核心变更**（基于v3.0）：
> 1. **复审人操作直接生效**：复审人的导入、更新、删除操作无需弹出提交审核弹窗，直接生效。
> 2. **创建者按初审人权限对待**：创建者虽具备复审能力，但在"我的提交"Tab中按初审人角色展示（显示完整审核流程）。
> 3. **"我的提交"Tab优化**：
>    - 纯复审人：隐藏"审核状态"和"审核详情"列、隐藏筛选器
>    - 其他角色：显示所有提交记录（含已生效），提供完整筛选功能
> 4. **审核记录Tab优化**：
>    - "审核结果"改为"审核状态"（通过、驳回、审核中）
>    - 审核中记录显示当前应审核人的真实姓名（工号）
>    - 移除"审结环节"和重复的"提交时间"列
> 5. **审批流展示优化**：
>    - 只在实际进入复审环节时才显示"等待复审"节点
>    - 初审直接生效显示为"初审通过（直接生效）"并附带生效版本
>    - 详情弹窗底部新增完整审批流时间线
> 6. **去除冗余提示**：移除操作按钮的"（需审核）"字样、去除重复导入按钮
...
- **显示所有记录**（v4.0变更）：包括已生效的记录（之前会过滤 `approved` 状态）
...
> **v4.1 变更**：按日期分组显示 + 智能归档，提升大量版本场景下的浏览效率。
...
> v4.0变更：显示所有提交记录，包括审核中状态，不再仅限已审结记录。
...
2. **审核详情弹窗**（v4.0优化）：
   ...
   - 内容：
     ...
     - 新增 / 删除：单侧展示
     - **底部新增审批流时间线**（v4.0）：显示完整的提交 → 初审 → 复审流程
   - 操作（根据进入入口不同）：
     - 在「待初审」Tab 进入时：显示三个按钮「通过并生效」「通过并提交复审」「驳回」
     - 在「待复审」Tab 进入时：显示两个按钮「复审通过」「复审驳回」
     - 在「审核记录」或「我的提交」只读模式进入时：不显示审核按钮，仅展示审批流
...
5. **审批流弹窗**（v4.0优化 - 动态节点显示）：
   - 触发：点击「查看审批流」
   - 内容：完整链路的节点流转图，**节点根据实际流转动态显示**（v4.0变更）：
     - **提交节点**（始终显示）
     - **初审节点**：
...
- **其他角色**（创建者、初审人、维护人员）：执行增（导入文件）、删（删除文档）、改（重命名/更新文档）操作时弹出确认弹窗
...
- 标题：「提交审核」
- 说明文案：
  - 有复审人：「你的本次{新增/删除/更新}操作将提交审核，需经初审人审核（初审人可选择直接生效或提交复审），审核通过后才会正式生效。」
  - 无复审人：「你的本次{新增/删除/更新}操作将提交审核，需经初审人审核，审核通过后将直接生效。」
...
- 说明文案：
  - 有复审人且为初审人/创建者：
    - **双按钮模式**：
      - 按钮1：「确定生效」（黄色，初审直接生效，`skipSecondReview: true`）
      - 按钮2：「提交复审」（蓝色主按钮，提交给复审人审核）
    - 文案：「请选择生效方式：可直接生效（跳过复审），或提交复审人审核。」
  - 无复审人：「你的本次{新增/删除/更新}操作将直接生效。」（仅"确定生效"按钮）
- 变更说明输入框（`changeDescription`，必填）
- 操作按钮：「取消」/「确定生效」/「提交复审」
